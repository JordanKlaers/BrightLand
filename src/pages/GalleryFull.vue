...
onMounted(async () => {
  imageMap['commercial'] = Object.entries(loadImages_Commercial())
    .slice(0, 3)
    .map(([path, loader]) => ({
      path,
      loader,
    }))
  imageMap['residential'] = Object.entries(loadImages_Residential())
    .slice(0, 3)
    .map(([path, loader]) => ({
      path,
      loader,
...
<template>
  ...
  <div class="container-gallery-full">
    ...
    <div class="container-imgs" ref="containerRef">
      <LazyImage
        v-for="({ path, loader }, index) in imageMap.commercial"
        :key="path"
        :loader="loader"
    ...
    <div class="container-imgs">
      <LazyImage
        v-for="({ path, loader }, index) in imageMap.residential"
        :key="path"
        :loader="loader"
        :alt="`Gallery Image ${index + 1}`"
      />
